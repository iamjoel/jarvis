...
| 维度 | 正则方案 | LLM 方案 |
|------|----------|----------|
| 响应延迟 | 微秒级，无网络 IO | 数百毫秒，需 API 调用 |
| 成本 | 零 token 消耗 | 每次消耗 token |
| 确定性 | 完全可预测 | 存在随机性 |
| 覆盖深度 | 只能匹配显式词汇 | 可理解隐晦/反讽语气 |

在"每次用户输入都实时扫描"的场景下，正则是性价比远高于 LLM 的正确选择。

#### 正则如何匹配负面情绪？

正则情绪检测的核心思路是**关键词列表 + 词边界匹配 + 大小写不敏感**。Claude Code 的实现据社区分析包含以下几类模式：

**1. 粗口与强烈负面词（直接负面信号）**
```ts
/\b(wtf|fuck|shit|damn|hell|crap|ass)\b/i
```
`\b` 是词边界锚点，确保只匹配独立单词，避免误匹配 `classic` 中的 `ass`。`/i` 标志使匹配大小写不敏感（`WTF`、`Wtf` 均可命中）。

**2. 挫败感短语（组合词组匹配）**
```ts
/\b(frustrat(ed|ing|ion)|annoyed?|so\s+annoying|hate\s+this)\b/i
```
`(ed|ing|ion)` 是分组选择，一条规则同时覆盖 `frustrated`、`frustrating`、`frustration`。`\s+` 匹配一个或多个空白字符，允许 `so annoying` 或 `so  annoying`（双空格）都命中。

**3. 表达"不工作/不理解"的短语（间接负面信号）**
```ts
/(why\s+(won't|doesn't|can't|won't)\s+this|this\s+(doesn't|won't|can't)\s+work)/i
```
捕获 `why won't this work`、`why doesn't this work` 等变体。`\s+` 处理单词之间的间距差异。

**4. 重复字符（情绪放大信号）**
```ts
/[!?]{2,}/
```
连续 2 个以上的 `!` 或 `?`（如 `what??!`、`why!!!`）通常是情绪激动的标志。`{2,}` 表示"至少匹配 2 次"。

**5. 全大写单词（喊叫信号）**
```ts
/\b[A-Z]{4,}\b/
```
匹配长度 ≥ 4 的全大写单词（`THIS IS BROKEN`、`NOTHING WORKS`），排除正常缩略词（`API`、`CLI`）。
...
[正则引擎并行扫描多个 pattern]
    │
    ├─ 命中任意 pattern → isFrustrated = true
    │
    ▼
isFrustrated = true？
    ├─ 是 → 调整系统提示中的语气指令（更多共情，提供故障排查流程）
    │        记录至产品分析（UX 数据）
    └─ 否 → 正常响应流程
```

**关键设计细节**：
- 多个 pattern 以数组形式存储，通过 `patterns.some(p => p.test(input))` **并行短路求值**——一旦有一个命中即返回 `true`，不再扫描后续规则；
- 扫描发生在用户输入进入主处理管道**之前**，不阻塞响应生成；
- 结果仅影响系统提示中的**语气调节模块**（独立于核心逻辑），确保主功能不受干扰。

#### 局限性

- **无法识别隐晦语气**：`"sure, this is totally working as expected"` （反讽）会被误判为中性；
- **语言单一**：大量英文 pattern，中文、日文等非英语负面情绪几乎无法覆盖；
- **容易被绕过**：用户稍微改变表达方式（如故意打错字 `fustrating`）即可规避；
- **误报风险**：代码注释里出现的 `damn good regex` 也可能触发检测。
