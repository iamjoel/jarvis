...
关键技术会改变成本结构、权力分布和国家竞争，是理解未来格局的现实入口。
...
先区分要改善的是信息表达、Agent 协作、工作流编排还是软件实现，才能选对方法入口。

**如何与 LLM、Agent 和自动化系统协作？**

AI 工具正在改变知识工作流程，理解协作方式比追逐单个工具更能产生稳定收益。
...
Prompt 是把意图、上下文和约束传给模型的接口，清晰程度直接影响输出质量。
...
软件和界面是把想法变成可复用系统的主要方式，技术选择会影响长期迭代成本。
...
| [如何写出容易维护的 React 代码](./r/react-maintainable-code-scenarios.md) | 从纯渲染、state 所有权、Effect、Context、数据获取、组件 API、目录、性能与测试出发，用 18 个典型场景给出正反例、例外和来源 |
| [前端技术史](./f/frontend-history.md) | Web 前端技术脉络梳理 |
| [Web 后端整体技术发展史](./b/backend-history.md) | 主要后端语言、生态与演变路线 |
| [TypeScript 7 相比 TypeScript 6 增加了什么？](./t/typescript-7-vs-6.md) | 区分语言功能与工具链换代，梳理 Go 原生编译器、多线程、并行构建、LSP、watch、Unicode 类型变化及升级兼容性 |
| [AntV 信息图表](./a/antv-infographic.md) | 信息可视化与图表表达的相关资源 |
| [工厂类大屏 Dashboard：开源项目与酷炫 UI 设计](./f/factory-dashboard-open-source-ui-by-industry.md) | 按制造、能源、流程工业、物流、水务及农业食品医药分组，区分工业平台、可视化底座与视觉案例，并给出选型和落地清单 |

**还想继续扩展阅读？**

扩展阅读能把单点工具放回更大的技术、社会和认知背景中理解。
